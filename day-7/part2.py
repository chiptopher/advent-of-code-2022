...
class Folder(Item):
    ...
    def __str__(self):
        return self.name + '(' + str(self.size()) + '): ' + str([str(f) for f in self.contents])
...
with open('day-7/input.txt', 'r') as source:
    ...
    def find_of_size(folder) -> None:
        if folder.has_content():
            for content in folder.contents:
                find_of_size(content)

            potential_space = total_space - root.size() + folder.size()

            if potential_space > needed_space:
                found.append(folder)
